class TicketService
{
    ...
    public void SaveTicketsToTextFile(List<Ticket> tickets)
    {
        ...
        for (int i = 0; i < tickets.Count; i++)
        {
            ...
            lines.Add($"Ticket Number: ABC-{i + 1:000}");
            lines.Add($"Subject: {ticket.Subject}");
            lines.Add($"Description: {ticket.Description}");
            lines.Add($"Affected User: {ticket.AffectedUser}");
            lines.Add($"Affected Device: {ticket.Brand}");
            lines.Add($"Technician: {GetTechnician(ticket.Brand)}");
            lines.Add($"Priority: {GetPriorityLabel(ticket.Severity)} - {ticket.Severity}");
            lines.Add($"Status: {ticket.Status}");
            lines.Add($"Date of Occurrence: {ticket.Month}/{ticket.Day}/{ticket.Year}");
            lines.Add("----------------------------------------");
        }
